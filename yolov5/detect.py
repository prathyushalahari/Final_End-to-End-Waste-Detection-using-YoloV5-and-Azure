...
# # Fix for Windows: Convert PosixPath to string
# if os.name == 'nt':  # Check if running on Windows
#     def Path(x): return str(x)  # Override Path with string conversion
...
if str(ROOT) not in sys.path:
    sys.path.append(str(ROOT))  # Add ROOT to PATH
ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # Convert to relative path
...
def run(
    ...
    project=ROOT / "yolov5/output_detect",  # Change the output directory
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        if save_format == 0:
                            coords = (
                                (xyxy2xywh(torch.tensor(xyxy).view(
                                    1, 4)) / gn).view(-1).tolist()
                            )  # normalized xywh
                        else:
                            coords = (torch.tensor(xyxy).view(
                                1, 4) / gn).view(-1).tolist()  # xyxy
                        # label format
                        line = (cls, *coords,
                                conf) if save_conf else (cls, *coords)
...
def parse_opt():
    ...
    parser.add_argument("--project", default=ROOT /
                        "yolov5/output_detect/", help="save results to project/name")
